...
export default function StickyProjects() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Trigger active project index as user scrolls past each left block
  ...
  return (
    ...
    >
      ...
      {/* Split Grid: Left column scrolls through Orbit, Otto, CineFlix; Right column stays sticky throughout */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-start relative min-h-screen">
        ...
        <div className="lg:col-span-6 space-y-56 lg:space-y-72 pb-24">
            ...
            return (
              <div
                ...
                className={`transition-all duration-500 will-change-transform ${
                  isActive ? "opacity-100 scale-100" : "opacity-20 lg:scale-[0.98]"
                }`}
              >
                {/* 1. Small Violet Index + Category Micro-Label */}
                <div className="flex items-center gap-3 text-xs sm:text-sm font-mono uppercase tracking-widest mb-6">
                ...
                {/* 5. Wrapped Row of Individual Tech Tag Pills (Plain bordered rectangles, text only) */}
                <div className="flex flex-wrap gap-2.5 sm:gap-3 mb-12">
                  {project.tech.map((t) => (
                    <span
                      key={t}
                      className="font-mono text-xs sm:text-sm px-3.5 py-1.5 rounded-md bg-[#141414] text-neutral-300 border border-neutral-800"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <div className="lg:hidden mt-12 w-full aspect-[16/10] rounded-2xl overflow-hidden border border-neutral-800 bg-[#141414] shadow-2xl">
                  <img
                    src={project.image}
        ...
        {/* RIGHT COLUMN: Continuously Pinned Sticky Browser Preview Panel */}
